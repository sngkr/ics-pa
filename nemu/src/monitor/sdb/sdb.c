...
static struct {
  ...
  int (*handler)(char *);
    ...
    {"si",
     "Let the program step into N instructions and then pause the "
     "execution,When N is not given, the default is 1",
     cmd_step},
    {"info", "info r :Print register status", cmd_info},
    {"x",
     "Calculate the value of the expression EXPR, use the result as the "
...
static int cmd_info(char *args) {
  ...
  if (arg == NULL) {
    /* no argumemt given  */
    printf("info: usage error: Parameter r required\n");
  } else {
    if (strcmp(arg, "r") == 0) {
      isa_reg_display();
    }
